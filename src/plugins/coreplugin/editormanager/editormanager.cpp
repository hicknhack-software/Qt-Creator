...
bool EditorManagerPrivate::skipOpeningBigTextFile(const FilePath &filePath)
{
    ...
    if (fileSizeInMB > systemSettings().bigFileSizeLimitInMB()
        ...
        auto doNotAskAgain = [] {
            systemSettings().warnBeforeOpeningBigFiles.setValue(false);
            systemSettings().writeSettings();
        };

        QMessageBox::StandardButton clickedButton = CheckableMessageBox::question(
            title, text, {shouldAskAgain, doNotAskAgain, CheckableDecider::OnAnyButton});
    ...
    connect(
        ...
        [area = QPointer<EditorArea>(area)] {
            if (ExtensionSystem::PluginManager::isShuttingDown())
                return;
            // the area can be gone by the time this queued call arrives, e.g.
            // when the mode that owns it is taken down
            if (!area)
                return;
    ...
    EditorManager::ContextMenuFlags flags)
{
    ...
    if (!flags.testFlag(EditorManager::HidePathActions)) {
        ...
        if (HostOsInfo::isWindowsHost()) {
            addMenuAction(contextMenu,
                          ::Core::Tr::tr("Open With Visual Studio"),
                          enabled,
                          d,
                          [filePath] { EditorManager::instance()->openWithVisualStudio(filePath); });
        }
