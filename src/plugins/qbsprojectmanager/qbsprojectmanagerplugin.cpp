...
bool QbsProjectManagerPlugin::initialize(const QStringList &arguments, QString *errorMessage)
{
    ...
    m_generateVs2022Ctx = new QAction(tr("Generate VisualStudio2022 Project"), this);
    ...
    m_debugWithVs2022Ctx = new QAction(tr("Debug Target with VisualStudio2022"), this);
...
void QbsProjectManagerPlugin::generateVs2022Project()
{
    ...
    proc.setTimeoutS(10 * 60); // note: Qbs has to build a graph what might be slow for large projects

    auto cmd = Core::ShellCommand(project->rootProjectDirectory(), Utils::Environment::systemEnvironment());
    cmd.addFlags(Core::ShellCommand::ShowStdOut | Core::ShellCommand::ShowSuccessMessage);
    cmd.runCommand(proc, commandLine);
    auto output = proc.allOutput();
    if (!output.isEmpty()) {
        Core::MessageManager::writeFlashing(output);
    }
    Core::MessageManager::writeSilently(proc.exitMessage());
}
